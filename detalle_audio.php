...
if (!empty($id)) {
    $consulta = $conexion->prepare("SELECT * FROM producto WHERE id_prod = ? AND tipo_prod = 'audio'");
...
<head>
    ...
    <style>
        ...
        .preview-audio { margin-top: 20px; text-align: center; }
        .preview-audio img { width: 100px; }
        .descripcion { margin-top: 30px; }

        /* Barra de audio (copiada del primer código) */
        .barra-audio {
            margin-top: 15px;
            width: 300px;
            height: 10px;
...
<div class="detalle">
    <div class="preview">
        ...
        <div class="barra-audio"></div> <!-- AQUI SE AÑADE LA BARRA DE AUDIO -->
    </div>
    <div>
        <p>ID: <?= $id_prod ?></p>
        <h2><?= $nombre ?></h2>
        <form method="POST">
            ...
            <button type="submit" class="corazon" style="font-size: 24px; color: <?= isset($_SESSION['deseos'][$id_prod]) ? 'red' : 'gray' ?>;">
